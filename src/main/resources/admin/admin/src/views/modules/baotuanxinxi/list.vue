<template>
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">线路图片</label>
						<el-input v-model="searchForm.xianlutupian" placeholder="线路图片" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">出发日期</label>
						<el-date-picker v-model="searchForm.chufariqistart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="出发日期起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.chufariqiend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="出发日期结束"></el-date-picker>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"right","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					<el-button class="add" v-if="isAuth('baotuanxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('baotuanxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu8" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('baotuanxinxi','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('baotuanxinxi','线路报名人数')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('baotuanxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('baotuanxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('baotuanxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('baotuanxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="xianlumingchengChart1" style="width:100%;height:600px;"></div>
...
import AddOrUpdate from "./add-or-update";
	export default {
		...
		methods: {
			...
			payHandler(row){
				this.$storage.set('paytable','baotuanxinxi');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var xianlumingchengChart1 = echarts.init(document.getElementById("xianlumingchengChart1"),'blue');
        this.$http({
            url: "baotuanxinxi/group/xianlumingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                xianlumingchengChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xianlumingchengChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xianlutupian!='' && this.searchForm.xianlutupian!=undefined){
            params['xianlutupian'] = '%' + this.searchForm.xianlutupian + '%'
          }
           if(this.searchForm.chufariqistart!='' && this.searchForm.chufariqistart!=undefined ){
            params['chufariqistart'] = this.searchForm.chufariqistart
          }
          if(this.searchForm.chufariqiend!='' && this.searchForm.chufariqiend!=undefined){
            params['chufariqiend'] = this.searchForm.chufariqiend
          }
	...
	baotuanxinxistatusChange(e,row){
		if(row.status==0){
