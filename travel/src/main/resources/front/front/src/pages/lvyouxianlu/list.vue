...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #C7C7C7","padding":"0","margin":"20px auto 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px","display":"inline-block"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>线路名称：</div>
        <el-input v-model="formSearch.xianlumingcheng" placeholder="线路名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px","display":"inline-block"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>景点类型：</div>
        <el-input v-model="formSearch.jingdianleixing" placeholder="景点类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px","display":"inline-block"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>起点：</div>
        <el-input v-model="formSearch.qidian" placeholder="起点" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px","display":"inline-block"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>导游姓名：</div>
        <el-input v-model="formSearch.daoyouxingming" placeholder="导游姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('lvyouxianlu','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0","outline":"none","color":"#fff","borderRadius":"0","background":"#80BBCA","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/lvyouxianluAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"border":"2px solid #DF847F","width":"100%","padding":"40px 0 0","margin":"0px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
			<div :style='{"border":"1px solid #DF847F","padding":"5px","margin":"0 2% 20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"16%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.xianlutupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"180px","order":"1"}' v-if="item.xianlutupian && item.xianlutupian.substr(0,4)=='http'" :src="item.xianlutupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.xianlutupian?item.xianlutupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"180px","order":"1"}' v-else :src="baseUrl + (item.xianlutupian?item.xianlutupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","color":"#000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"16px","order":"3"}' class="name ">{{item.xianlumingcheng}}</div>
				<div :style='{"padding":"0 10px","color":"#000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"16px","order":"3"}' class="name ">起点:{{item.qidian}}</div>
				<div :style='{"padding":"0 10px","color":"#000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"16px","order":"3"}' class="name ">终点:{{item.zhongdian}}</div>
				...
				<div :style='{"width":"40%","padding":"0 10px","textAlign":"center","display":"inline-block","order":"8"}'>
				  <span class="icon iconfont icon-geren16" :style='{"margin":"0 2px 0 0","lineHeight":"2","fontSize":"14px","color":"#909090"}'></span>
				  <span class="text" :style='{"color":"#909090","lineHeight":"2","fontSize":"14px"}'>{{item.daoyougonghao}}</span>
				</div>
				<div :style='{"width":"calc(100% / 3)","padding":"0 10px","textAlign":"center","display":"inline-block","order":"4"}'>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/remenjingdian/jingdianmingcheng').then(res => {
			if (res.data.code == 0) {
				this.jingdianmingchengOptions = res.data.data;
				this.selectOptionsList.push({name:'景点名称',list:this.jingdianmingchengOptions,tableName: 'jingdianmingcheng',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
