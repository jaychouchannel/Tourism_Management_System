...
<div>
	<div :style='{"padding":"20px 20px 60px","margin":"10px auto","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20231217/d400927926594c4db68c914d1fab2fc3.png)","width":"90%","backgroundSize":"100% 40%","backgroundPosition":"bottom center","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/meishicanting"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 5%","margin":"10px auto","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","position":"relative"}'>
		<div class="attr" :style='{"width":"55%","padding":"0 20px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>特色菜品</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.tesecaipin}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>营业时间</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.yingyeshijian}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>人均消费</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.renjunxiaofei}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>餐厅地址</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.cantingdizhi}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>联系电话</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.lianxidianhua}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"15px","height":"40px"}' v-if="btnAuth('meishicanting','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('meishicanting','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
					<!-- crossOptButtonStatusColumns  -->
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#DF847F","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('meishicanting','立即预约')" @click="onAcross('cantingyuyue','','','','')" type="warning">立即预约</el-button>
				</div>
			</div>
		</div>
		...
		<el-tabs class="detail" :style='{"border":"0","boxShadow":"none","padding":"0 20px","background":"#FFF","flex":"1","width":"100%","order":"6"}' v-model="activeName" type="border-card">
																<el-tab-pane label="餐厅简介" name="first">
				<div v-html="detail.cantingjianjie"></div>
			</el-tab-pane>
  ...
  export default {
    ...
    methods: {
      ...
      async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
        localStorage.setItem('crossTable',`meishicanting`);
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('meishicanting/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'meishicanting', userid: localStorage.getItem('frontUserid')}}).then(res => {
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("frontToken")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'meishicanting', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'meishicanting', userid: localStorage.getItem('frontUserid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discussmeishicanting/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussmeishicanting/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussmeishicanting/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
				this.form.score = 0
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/meishicantingAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此美食餐厅？')
			  .then(_ => {
			    this.$http.post('meishicanting/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
