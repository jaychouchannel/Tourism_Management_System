...
<div>
	<div :style='{"padding":"20px 20px 60px","margin":"10px auto","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20231217/d400927926594c4db68c914d1fab2fc3.png)","width":"90%","backgroundSize":"100% 40%","backgroundPosition":"bottom center","backgroundRepeat":"no-repeat"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/xianluleixing"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"55%","padding":"0 20px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0","background":"#f5f5f5","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>线路类型</div>
					<div  :style='{"padding":"8px 10px 0","color":"#888","textAlign":"left","flex":"1","fontSize":"15px","lineHeight":"24px","height":"auto"}'>{{detail.xianluleixing}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"#fff","width":"auto","lineHeight":"40px","fontSize":"15px","height":"40px"}' v-if="btnAuth('xianluleixing','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #DF847F","cursor":"pointer","padding":"0 30px","margin":"0 10px 10px 0","outline":"none","color":"#DF847F","borderRadius":"0","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('xianluleixing','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/xianluleixingAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此线路类型？')
			  .then(_ => {
			    this.$http.post('xianluleixing/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
