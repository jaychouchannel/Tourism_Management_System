...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #C7C7C7","padding":"0","margin":"20px auto 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"auto","justifyContent":"center","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px","display":"inline-block"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>线路类型：</div>
        <el-input v-model="formSearch.xianluleixing" placeholder="线路类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('xianluleixing','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0","outline":"none","color":"#fff","borderRadius":"0","background":"#80BBCA","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/xianluleixingAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"border":"2px solid #DF847F","width":"100%","padding":"40px 0 0","margin":"0px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
			<div :style='{"border":"1px solid #DF847F","padding":"5px","margin":"0 2% 20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"16%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.image)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"180px","order":"1"}' v-if="item.image && item.image.substr(0,4)=='http'" :src="item.image.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.image?item.image.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"180px","order":"1"}' v-else :src="baseUrl + (item.image?item.image.split(',')[0]:'')" class="image" />
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.xianluleixing != '') searchWhere.xianluleixing = '%' + this.formSearch.xianluleixing + '%';
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`xianluleixing/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/xianluleixingDetail', query: params});
      },
